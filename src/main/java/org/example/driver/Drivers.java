...
public final class Drivers {
    ...
    public static AppiumDriver<MobileElement> createAndroidDriver() {
        try {
            ...
            capability.setCapability(AndroidMobileCapabilityType.CHROMEDRIVER_PORT, getConfig(ConfigJson.ANDROID_CHROMEDRIVER_PORT)); // For Web view/Chrome browser to launch the browser on different port
            capability.setCapability(AndroidMobileCapabilityType.SYSTEM_PORT, getConfig(ConfigJson.ANDROID_SYSTEM_PORT)); // To set different port for each thread - This port is used to communicate with UiAutomator2
            if (getConfig(ConfigJson.ANDROID_EMULATOR).equalsIgnoreCase("yes")) {
                capability.setCapability(AndroidMobileCapabilityType.AVD, getConfig(ConfigJson.ANDROID_DEVICE_NAME));
    ...
    public static AppiumDriver<MobileElement> createIOSDriver() {
        try {
            ...
            capability.setCapability(IOSMobileCapabilityType.WDA_LOCAL_PORT, getConfig(ConfigJson.IOS_WDA_LOCAL_PORT)); // To set different port for each thread - This port is used to communicate with WebDriverAgent driver
            capability.setCapability(CapabilityType.BROWSER_NAME, MobileBrowserName.SAFARI);
            capability.setCapability("webkitDebugProxyPort", getConfig(ConfigJson.IOS_WEBKIT_DEBUG_PROXY_PORT)); // For web view/Safari browser testing on real device
